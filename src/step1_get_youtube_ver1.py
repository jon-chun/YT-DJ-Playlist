...
def load_config():
    ...
    while True:
        ...
        if root_dir == root_dir.parent:
            ...
            # Set root_dir to the directory containing the script
            root_dir = current_path.parent.parent
            print(f"Using script parent directory as root: {root_dir}")
            break
...
def download_video(video_url, output_dir, type, size, language, video_format, audio_format, log_level):
    ...
    # Format specification - more flexible to increase success rate
    if type == 'video':
        format_spec = f'bestvideo[height<={size}]+bestaudio/best[height<={size}]/best'
    else:  # audio
        format_spec = f'bestaudio/best'
...
def get_youtube(url=None, type='video', size='760', language='en', video='mp4', audio='mp3', log_level='INFO'):
    ...
    # Set up output directory at project root
